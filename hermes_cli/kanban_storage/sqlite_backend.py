...
class SqliteBackend:
    ...
    def open_connection(self, config: StorageConfig) -> sqlite3.Connection:
        ...
        # Keep the upstream fail-closed corruption guard after extracting the
        # SQLite implementation out of kanban_db.py. Import lazily to avoid a
        # module-cycle during selector initialization.
        try:
            from hermes_cli.kanban_db import _guard_existing_db_is_healthy
    ...
    def write_txn(self, conn: sqlite3.Connection):
        ...
        on clean exit, rolls back on exception, never swallows. The rollback
        guard preserves the original exception when SQLite has already
        auto-rolled-back, and the post-commit file-length invariant catches
        torn-extend corruption before later readers silently ingest it.
        """
        from hermes_cli.kanban_db import _execute_boundary_with_retry

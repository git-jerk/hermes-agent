...
def _clean_env(monkeypatch):
    ...
    monkeypatch.setenv("HERMES_DISABLE_CODEX_1P", "1")
    ...
    monkeypatch.setattr(
        "hermes_cli.auth._read_codex_tokens_from_1p",
        lambda auth_store=None: None,
        raising=False,
    )
...
class TestCodexAuxiliaryAdapterTimeout:
    ...
    def test_enforces_total_timeout_while_stream_keeps_emitting_events(self):
        ...
        fake_client = SimpleNamespace(responses=FakeResponses(), close=stop_stream.set)
        adapter = _CodexCompletionsAdapter(fake_client, "gpt-5.5")

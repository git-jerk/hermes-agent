...
def test_explicit_board_ignores_inherited_direct_db_env(kanban_home, tmp_path, monkeypatch):
    """Global --board is more explicit than a worker-pinned HERMES_KANBAN_DB."""
    kc.run_slash("boards create beta")
    rogue_db = tmp_path / "worker-pinned.db"
    monkeypatch.setenv("HERMES_KANBAN_DB", str(rogue_db))

    out = kc.run_slash("--board beta create 'beta scoped' --assignee alice --json")
    payload = json.loads(out)
    assert payload["title"] == "beta scoped"

    # Read back with the inherited direct-db pin removed, matching a clean
    # operator shell. If the CLI failed to clear HERMES_KANBAN_DB during the
    # command, the beta board will be empty and the rogue DB will contain the row.
    monkeypatch.delenv("HERMES_KANBAN_DB", raising=False)
    with kb.connect(board="beta") as conn:
        beta_titles = [task.title for task in kb.list_tasks(conn)]
    assert "beta scoped" in beta_titles

    if rogue_db.exists():
        with kb.connect(db_path=rogue_db) as conn:
            rogue_titles = [task.title for task in kb.list_tasks(conn)]
        assert "beta scoped" not in rogue_titles


def test_storage_status_ignores_inherited_direct_db_env(kanban_home, tmp_path, monkeypatch):
    """Storage board enumeration must not collapse every slug onto worker DB."""
    kc.run_slash("boards create beta")
    rogue_db = tmp_path / "worker-pinned.db"
    monkeypatch.setenv("HERMES_KANBAN_DB", str(rogue_db))

    out = kc.run_slash("storage status --board beta --json")
    payload = json.loads(out)

    assert payload[0]["board"] == "beta"
    assert payload[0]["backend"] == "sqlite"
    assert payload[0]["sqlite_path"].endswith("/kanban/boards/beta/kanban.db")
    assert payload[0]["sqlite_path"] != str(rogue_db)


def test_boards_create_ignores_inherited_direct_db_env(kanban_home, tmp_path, monkeypatch):
    """Board management should not initialize/report a worker-pinned DB path."""
    rogue_db = tmp_path / "worker-pinned.db"
    monkeypatch.setenv("HERMES_KANBAN_DB", str(rogue_db))

    out = kc.run_slash("boards create beta")

    assert str(rogue_db) not in out
    assert not rogue_db.exists()
    assert (kanban_home / "kanban" / "boards" / "beta" / "kanban.db").exists()
